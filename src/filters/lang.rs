...
pub enum Language {
    Rust, Python, JavaScript, TypeScript, Go, C, Cpp, Java, Ruby, Shell,
...
impl Language {
    ...
    pub fn from_ext(ext: &str) -> Self {
        match ext.to_lowercase().as_str() {
            ...
            | "toml" | "xml" | "csv" | "tsv"
            | "graphql" | "gql" | "sql" | "md"
            | "markdown" | "txt" | "env" | "lock"  => Self::Data,
            _                                       => Self::Unknown,
        }
    }
...
fn minimal(content: &str, lang: Language) -> String {
    ...
    for line in content.lines() {
        ...
        if let (Some(start), Some(end)) = (p.block_start, p.block_end) {
            if !in_docstring && t.contains(start)
                && !t.starts_with(p.doc_block_start.unwrap_or("###"))
            {
                in_block = true;
            }
            if in_block {
                if t.contains(end) { in_block = false; }
                continue;
            }
        }
        ...
        if in_docstring { out.push_str(line); out.push('\n'); continue; }
...
fn aggressive(content: &str, lang: Language) -> String {
    ...
    for line in base.lines() {
        ...
        if t.starts_with("const ") || t.starts_with("static ") || t.starts_with("pub const ")
